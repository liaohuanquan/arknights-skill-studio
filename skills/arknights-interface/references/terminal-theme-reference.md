...
| Atmosphere | Neutral industrial silhouettes, faint surface texture, and sparse dust behind opaque content surfaces. Set the particle count to `floor((width + height) / 38)`. Randomize leftward speed from `1–2px` and upward speed from `0.01–1.01px` per 60Hz-equivalent frame. Use `0.5–2px` horizontal radii, vertical radii at `0.3–1.3` times that size, and random `0–3px` soft halos with slight offsets to imply depth. Pause the canvas when the tab is hidden. |
